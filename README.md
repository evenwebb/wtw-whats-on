...
- Writes `whats_on_data.json` and regenerates `index.html` every run.
...
1. **Settings → Pages** — Source: **Deploy from a branch** → Branch: **main** → **/ (root)** → Save.  
   Site: `https://<username>.github.io/wtw-whats-on/`.
...
3. **Workflow** — `.github/workflows/whats_on_html.yml` runs daily (09:00 UTC) and on manual trigger. It commits `whats_on_data.json`, `index.html`, `.tmdb_cache.json`, `.whats_on_fingerprint`, `posters/`, `certs/` and `icons/` only when the data fingerprint changes.
...
| `index.html` | Single-page UI; generated each run |
...
| `certs/` | BBFC-style age-rating images |
| `icons/` | 3D overlay image for posters |
